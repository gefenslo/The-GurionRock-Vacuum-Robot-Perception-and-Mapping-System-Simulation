...
/**
 * invariants:
 * @inv messages != null (message types to subscribers mapping must exist)
 * @inv queues != null (microservice to message queue mapping must exist)
 * @inv subscriptions != null (microservice to callbacks mapping must exist)
 */
     ...
     */
     /**
     * @pre m != null (microservice must not be null)
     * @pre type != null (event type must not be null)
     * @post messages.get(type) contains m (microservice is added to subscribers list)
     */
     ...
     */
     /**
     * @pre m != null (microservice must not be null)
     * @post queues contains m as key (microservice has an allocated queue)
     * @post queues.get(m) is empty (newly allocated queue is empty)
     */
    ...
    /**
     * @pre m != null (microservice must not be null)
     * @pre m is registered (microservice must be registered)
     * @post if message returned, it was removed from m's queue
     */
